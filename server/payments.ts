 ...
 *
 * idempotencyKey is generated by the server from the order identity and must
 * be forwarded to a real provider so retries cannot create duplicate charges.
 */
export async function createPixCharge(input: { orderId: number; amount?: string; pixKey: string; idempotencyKey: string }): Promise<PixChargeResult> {
  ...
  const response = await fetch(`${ENV.pixApiUrl.replace(/\/$/, "")}/charges`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${ENV.pixApiKey}`,
      "Content-Type": "application/json",
      "Idempotency-Key": input.idempotencyKey,
    },
    body: JSON.stringify({
      externalReference: `pediu-order-${input.orderId}`,

  ...
  pediu: router({
    ...
    checkout: router({
      quote: protectedProcedure.input(z.object({
        ...
        items: z.array(z.object({ productId: z.number().int().positive(), quantity: z.number().int().positive().max(50), note: z.string().trim().max(500).optional() })).min(1),
    ...
    orders: router({
      ...
      create: protectedProcedure.input(z.object({ idempotencyKey: z.string().trim().min(8).max(160), storeId: z.number().int().positive(), total: z.string().regex(/^\d+(\.\d{1,2})?$/), paymentMethod: z.enum(["pix", "card", "cash", "fiado"]).default("pix"), addressId: z.number().int().positive().optional(), deliveryAddress: z.string().trim().max(255).optional(), couponCode: z.string().trim().min(1).max(40).optional(), items: z.array(z.object({ productId: z.number().int().positive(), quantity: z.number().int().positive().max(50), unitPrice: z.string().regex(/^\d+(\.\d{1,2})?$/), note: z.string().trim().max(500).optional() })).min(1) })).mutation(async ({ ctx, input }) => {
        ...
        if (input.paymentMethod === "fiado") { const customer = await db.getCustomerCreditByUser(input.storeId, ctx.user.id); if (!customer) throw new Error("Cliente não habilitado para fiado nesta loja"); const orderId = await db.createOrderWithFiado({ customerId: ctx.user.id, storeId: input.storeId, total: serverTotal, couponCode, discount, deliveryAddress, idempotencyKey }, input.items.map((item, i) => ({ productId: item.productId, quantity: item.quantity, unitPrice: String(products[i]!.price), ...(item.note ? { note: item.note } : {}) })), customer.id, input.storeId); try { await db.createDeliveryEvent({ orderId, eventType: "Pendente" }); } catch (error) { console.warn("[Orders] Failed to persist initial delivery event:", error); } try { await sendPushToUser(store.ownerId, "Novo pedido", `O pedido #${orderId} foi recebido e está pendente de aceite.`, { type: "order", orderId, status: "Pendente" }); } catch (error) { console.warn("[Orders] Failed to notify store owner about new fiado order:", error); } return { orderId, paymentId: null, status: "Pendente" as const }; }
        const { orderId, paymentId } = await db.createOrderWithPayment({ customerId: ctx.user.id, storeId: input.storeId, total: serverTotal, couponCode, discount, deliveryAddress, idempotencyKey }, input.items.map((item, i) => ({ productId: item.productId, quantity: item.quantity, unitPrice: String(products[i]!.price), ...(item.note ? { note: item.note } : {}) })), input.paymentMethod);
        try { await db.createDeliveryEvent({ orderId, eventType: "Pendente" }); } catch (error) { console.warn("[Orders] Failed to persist initial delivery event:", error); } try { await sendPushToUser(store.ownerId, "Novo pedido", `O pedido #${orderId} foi recebido e está pendente de aceite.`, { type: "order", orderId, status: "Pendente" }); } catch (error) { console.warn("[Orders] Failed to notify store owner about new order:", error); }
      ...
      status: protectedProcedure.input(z.object({ orderId: z.number().int().positive(), status: orderStatusSchema })).mutation(async ({ ctx, input }) => {
        ...
        if (!isOwner) { if (input.status !== "Cancelado" || !canCustomerCancelOrder(order.status)) throw new Error("O cliente só pode cancelar pedidos ainda não preparados"); await db.updateOrderStatus(input.orderId, input.status); try { await db.createDeliveryEvent({ orderId: input.orderId, eventType: input.status }); } catch (error) { console.warn("[Orders] Failed to persist cancellation event:", error); } await db.cancelPendingPaymentForOrder(input.orderId); const store = await db.getStoreById(order.storeId); if (store) { try { await sendPushToUser(store.ownerId, "Pedido cancelado", `O pedido #${order.id} foi cancelado pelo cliente.`, { type: "order", orderId: order.id, status: input.status }); } catch (error) { console.warn("[Orders] Failed to notify store owner about cancellation:", error); } } return { success: true as const }; }
        if (!canTransitionOrder(order.status, input.status)) throw new Error(`Transição de pedido inválida: ${order.status} → ${input.status}`); await db.updateOrderStatus(input.orderId, input.status); try { await db.createDeliveryEvent({ orderId: input.orderId, eventType: input.status }); } catch (error) { console.warn("[Orders] Failed to persist delivery event:", error); } try { await sendPushToUser(order.customerId, "Atualização do pedido", `Seu pedido #${order.id} agora está: ${input.status}.`, { type: "order", orderId: order.id, status: input.status }); } catch (error) { console.warn("[Orders] Failed to notify customer about status change:", error); } return { success: true as const };
      }),
    }),
    payments: router({

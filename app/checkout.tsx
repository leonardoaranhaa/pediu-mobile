...
export default function CheckoutScreen() {
  ...
  return <Page title="Finalizar pedido" eyebrow="CHECKOUT" back>
    <Card><Text style={s.sectionTitle}>Itens</Text>{cart.items.map((item) => <View key={item.productId} style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 5 }}><Text style={{ flex: 1 }}>{item.quantity}× {item.name}</Text><Text style={{ fontWeight: "800" }}>R$ {(item.price * item.quantity).toFixed(2).replace(".", ",")}</Text></View>)}</Card>
    ...
    <Card><Text style={s.sectionTitle}>Resumo</Text><Text style={s.muted}>Subtotal local: R$ {cart.subtotal.toFixed(2).replace(".", ",")}</Text><Text style={s.muted}>Entrega e descontos: calculados pelo servidor</Text><Text style={{ color: PEDIU.ink, fontSize: 21, fontWeight: "900", marginTop: 8 }}>Total final: calculado pelo servidor</Text></Card>
    <PrimaryButton title={submitting ? "Processando..." : "Confirmar pedido"} onPress={confirm} disabled={!address.trim() || submitting} />
  </Page>;
}
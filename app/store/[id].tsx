...
export default function StoreScreen() {
  ...
  return <Page title={storeName} eyebrow="CATÁLOGO" back>
    ...
    {query.isLoading ? <LoadingState label="Carregando catálogo..." /> : query.isError ? <ErrorState title="Não foi possível carregar o catálogo" onRetry={() => query.refetch()} /> : products.length === 0 ? <EmptyState title="Catálogo vazio" description="Este estabelecimento não possui produtos disponíveis no momento." /> : <>
      ...
      {visible.map((product, index) => <Stagger key={product.id} index={index}><Card><PressFeedback onPress={() => router.push({ pathname: "/product/[id]", params: { id: String(product.id) } })}><View><Text style={s.sectionTitle}>{product.name}</Text><Text style={s.muted}>{product.description || product.category}</Text><Text style={{ color: PEDIU.coral, fontSize: 18, fontWeight: "900", marginVertical: 8 }}>R$ {Number(product.price).toFixed(2).replace(".", ",")}</Text></View></PressFeedback><PrimaryButton title="Ver produto" onPress={() => router.push({ pathname: "/product/[id]", params: { id: String(product.id) } })} /></Card></Stagger>)}
    </>}
  </Page>;
}
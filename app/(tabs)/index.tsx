...
export default function HomeScreen() {
  ...
  const [mascotReaction, setMascotReaction] = useState<MascotReaction>("hungry");
  ...
  const showMascotMoment = (nextReaction: MascotReaction, duration = 4600) => {
    ...
    mascotResetTimer.current = setTimeout(() => setMascotReaction("hungry"), duration);
  };
    ...
    <ScreenContainer containerClassName="bg-[#FFF8F1]" edges={["top", "left", "right"]}>
      <Animated.View style={[styles.appShell, { opacity: screenOpacity, backgroundColor: theme.canvas }]}>
        ...
        {role === "customer" ? (
          <>
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              ...
              {customerTab === "profile" && (
                <><CustomerProfile user={user} isAuthenticated={isAuthenticated} theme={theme} customization={customization} notifications={notificationsQuery.data ?? []} onReadNotification={(id) => markNotificationMutation.mutate({ notificationId: id })} onLogin={requestLogin} onLogout={() => void logout()} onSellerMode={enterSellerMode} /><AuthPanel user={user} isAuthenticated={isAuthenticated} onLogin={requestLogin} onLogout={() => void logout()} /></>
              )}
            </ScrollView>
        ...
        <Modal visible={showCart} transparent animationType="slide" onRequestClose={() => setShowCart(false)}>
          <View style={styles.modalBackdrop}><View style={[styles.sheet, styles.sheetTall]}><ScrollView style={styles.sheetScroll} contentContainerStyle={styles.sheetContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
...
function CustomerProfile({ user, isAuthenticated, theme, customization, notifications, onReadNotification, onLogin, onLogout, onSellerMode }: { user: { name: string | null; email: string | null } | null; isAuthenticated: boolean; theme: AppTheme; customization: { mascotStyle: "classic" | "ocean" | "sunset"; mascotEnabled: boolean; motionEnabled: boolean }; notifications: { id: number; title: string; body: string; readAt: Date | null }[]; onReadNotification: (id: number) => void; onLogin: () => void; onLogout: () => void; onSellerMode: () => void }) {

...
const liveProducts = useMemo(() => (marketplaceQuery.data?.items ?? []).map((product) => ({
  ...
  const addToCart = (product: Product) => {
    ...
    if (mascotResetTimer.current) clearTimeout(mascotResetTimer.current);
    mascotResetTimer.current = setTimeout(() => setMascotReaction("hungry"), 2200);
    Animated.sequence([
  ...
  return (
    <ScreenContainer containerClassName="bg-[#FFF8F1]" edges={["top", "left", "right"]}>
      <Animated.View style={[styles.appShell, { opacity: screenOpacity, backgroundColor: theme.canvas }]}>
        ...
        {role === "customer" && customization.mascotEnabled ? <View style={styles.mascotDock}><PediuMascot theme={theme} styleId={customization.mascotStyle} reaction={mascotReaction} motionEnabled={customization.motionEnabled} compact onPress={() => notify(mascotReaction === "happy" ? "O mascote adorou essa escolha" : globalCartCount ? "Mais um pouquinho e fechamos seu pedido" : "Estou com fome de uma boa escolha")} /></View> : null}

  ...
  return <View style={{ gap: 11 }}>
    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={close}>
      <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(11,28,34,0.58)" }}>
        <Animated.View style={{ maxHeight: "92%", backgroundColor: theme.card, borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingHorizontal: 20, paddingTop: 11, paddingBottom: 24, transform: [{ translateY: modalProgress.interpolate({ inputRange: [0, 1], outputRange: [520, 0] }) }] }}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 15, paddingTop: 10, paddingBottom: 8 }}>
            <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: theme.ink, borderRadius: 21, padding: 13, gap: 10 }}><PediuMascot theme={theme} styleId={customization.mascotStyle} reaction={customization.mascotEnabled ? "happy" : "idle"} motionEnabled={customization.motionEnabled} /><View style={{ flex: 1, gap: 4 }}><Text style={{ color: theme.highlight, fontSize: 10, fontWeight: "900", letterSpacing: 1 }}>PREVIEW VIVO</Text><Text style={{ color: PEDIU.white, fontSize: 16, lineHeight: 20, fontWeight: "900" }}>{customization.mascotEnabled ? "Seu Pediu está pronto para acompanhar você." : "O Pediu fica mais discreto, mas continua funcionando."}</Text><Text style={{ color: "#BCD0D1", fontSize: 11, lineHeight: 16 }}>{customization.mascotEnabled ? "Ele reage quando você escolhe seus itens." : "Você pode reativar o mascote quando quiser."}</Text></View></View>
